...
#working on it...the column names like AdmissionLocation CLINIC REFERRAL break the code... i think the data needs to be integers but i can change.

# First I adapt the df for the model placeing outcome at the end
xgTrain<-dataTrain %>% 
  mutate(gender= as.integer(gender))  %>%
  mutate(first_icu_stay = as.integer(first_icu_stay))  %>%
  mutate(AdmissionType_ELECTIVE = as.integer(AdmissionType_ELECTIVE))  %>%
  mutate(AdmissionType_EMERGENCY = as.integer(AdmissionType_EMERGENCY))  %>%
  #mutate(AdmissionLocation_CLINIC REFERRAL/PREMATURE = as.integer(AdmissionLocation_CLINIC REFERRAL/PREMATURE))  %>%
  mutate(AdmissionLocation_EMERGENCY ROOM ADMIT = as.integer(AdmissionLocation_EMERGENCY ROOM ADMIT))  %>%
  mutate(AdmissionLocation_PHYS REFERRAL/NORMAL DELI = as.integer(AdmissionLocation_PHYS REFERRAL/NORMAL DELI)) %>% 
  mutate(AdmissionLocation_TRANSFER FROM HOSP/EXTRAM = as.integer(AdmissionLocation_TRANSFER FROM HOSP/EXTRAM)) %>% 
  mutate(Ethnicity_10=as.integer(Ethnicity_10)) %>% 
  mutate(Ethnicity_2=as.integer(Ethnicity_2)) %>% 
  mutate(Ethnicity_7=as.integer(Ethnicity_7)) %>% 
...
classifier <- xgboost(data = as.matrix(xgTrain[-70]), label = xgTrain$outcome, nrounds = 2) #note to self 198

# Evaluating model performance with K-fold cross validation
folds = createFolds(xgTrain$outcome, k = 2)
cv = lapply(folds, function(x) {
  training_fold = xgTrain[-x, ]
  test_fold = xgTrain[x, ]
  classifier = xgboost(data = as.matrix(xgTrain[-70]), label = xgTrain$outcome, nrounds = 2)
  y_pred = predict(classifier, newdata = as.matrix(test_fold[-70]))
  y_pred = (y_pred >= 0.5)
