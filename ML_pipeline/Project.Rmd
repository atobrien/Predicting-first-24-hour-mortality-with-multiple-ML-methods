...
hds<-hds %>% 
  mutate(ethnicity=case_when(ethnicity=="ASIAN"~1,
                             ...
                             ethnicity=="NATIVE"~ 4, 
                             ethnicity=="NATIVE FEDERALLY RECOGNIZED TRIBE" ~ 4,
                             ethnicity=="NATIVE HAWAIIAN OR OTHER PACIFIC ISLANDER" ~ 4,
                             ethnicity=="NO_ANSWER"~ 6, 
                             ethnicity=="OTHER"~ 4, 
                             ethnicity=="PORTUGUESE"~4,
                             ethnicity=="WHITE"~5)) %>%
...
```{r}
dim(hds %>% filter((ANIONGAP_max>40|CHLORIDE_min<39|GLUCOSE_min<20|POTASSIUM_max>13.8)))
#there are 139 patients that have clinically hard to believe lab values

hds <- hds %>% filter(!(ANIONGAP_max>40|CHLORIDE_min<39|GLUCOSE_min<20|POTASSIUM_max>13.8))
```
...
hds <- hds %>% select(-c("los_icu","los_hospital","intime","outtime","subject_id","hadm_id","icustay_id",
                         ...
                         "hospital_expire_flag","ICUSTAY_AGE_GROUP")) %>% add_column(Ethnicity_7 = ifelse(hds$Ethnicity_2+hds$Ethnicity_5+hds$Ethnicity_6==0,1,0), .after = "Ethnicity_6")
...
select_significant_feature <- function(classifier){
  ethni <- c('Ethnicity_1','Ethnicity_2','Ethnicity_3')
  admissiontype <- c('AdmissionType_ELECTIVE','AdmissionType_EMERGENCY')
  admissionlocation <- c('`AdmissionLocation_CLINIC REFERRAL/PREMATURE`','`AdmissionLocation_EMERGENCY ROOM ADMIT`','`AdmissionLocation_PHYS REFERRAL/NORMAL DELI`','`AdmissionLocation_TRANSFER FROM HOSP/EXTRAM`')
  
  list_features <- summary(classifier)$coefficients[-1,4]<=0.05
  names <- names(list_features)[list_features]
  for (name in names){
  print(name)
    if (substr(name, 1, 9)=='Ethnicity'){
      names <- c(names,ethni)
    }
    if (substr(name,1,13)=='AdmissionType'){
      names <- c(names,admissiontype)
    }
    if (substr(name,1,18)=='`AdmissionLocation'){
      names <- c(names,admissionlocation)
    }
  }
  names <- unique(names)
  return(names)
...
logTrain %>% select(select_significant_feature(classifier))
...
xgTest<-dataTest %>%  
  mutate(outcome=icustay_expire_flag) %>%
  mutate(outcome=as.factor(icustay_expire_flag)) %>%
  select(-c(icustay_expire_flag, Ethnicity_7)) 
