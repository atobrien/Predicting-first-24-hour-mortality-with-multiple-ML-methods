...
# This is a function to install the required packages if you do not already have them installed. 
# Uncomment and run before knitting. 

#install packages if you don't have it
#list.of.packages <- c("klaR", "caTools","caret","tidyverse","lubridate","Amelia","stringr","FactoMineR","pROC","doParallel","xgboost","ggExtra", "ggthemes")
...
We first used a variety of exploratory plots, including histograms and barplots to understand the distribution of our data to identify outliers, missing data and the shape of distributions to potentially inform further methods. At its core, our research question is a classification problem, so we decided to use logistic regression along with tree based methods, including first a simple decision tree, then a random forest and a gradient boosted tree.  

# Final Analysis: What did you learn about the data? How did you answer the questions? How can you justify your answers?

We learned that we are able to develop a reasonable estimate with just first day readings in the ICU, but need to incorporate additional complexity in order to effectively implement this algorithm in a clinical setting. In our context, sensitivity refers to our ability to accuratelly predict patients who survived past their stay in the hospital among the total patients alive. Specificity refers to our ability to predict deaths among patients that died. Different scores are currently used in hospitals to predict mortality, for example SOFA, OASIS and SAPS II and we were able to outperform all of them. As mentioned at the beginning of the project, we sought to develop a decision support tool, and believe that our algorithm could be used to help clinicians better understand a patient's future prognosis.  
...
Let's look at the plot of when people die.
...
We create a flag for people who die within the first 24h in the ICU. We exclude them from our analysis 
```{r}
...
```{r}
hds %>%mutate(age=case_when(age==1~"17-28",
                               age==2~"29-38",
                               age==3~"39-48",
                               age==4~"49-58",
                               age==5~"59-68",
                               ...
                               age==7~"79-88",
                               age==8~"89-98")) %>%
  ggplot(aes(x=age), data=.)+
  geom_histogram( fill="navyblue", stat="count", alpha=0.4)+
  theme_bw()+
...
```{r}
# Plot distribution of ionized calcium vs serum calcium with a marginal Bar chart

# OSM ATOMIC SPECTRUM COLOR SCHEME
hds$hds_color[(2*hds$SODIUM_min) <= 200 & (2*hds$POTASSIUM_min) <=7.5] <- "#15051f"  #purple Quadrant 1
hds$hds_color[(2*hds$SODIUM_min) >= 200 &  (2*hds$SODIUM_min) <= 270 & (2*hds$POTASSIUM_min) <=7.5] <- "#44206e" #purple Quadrant 2
hds$hds_color[(2*hds$SODIUM_min) >= 270 & (2*hds$POTASSIUM_min) <=7.5]  <- "#2e6658" #green Quadrant 3
hds$hds_color[(2*hds$SODIUM_min) <= 200  & (2*hds$POTASSIUM_min) >=7.5 & (2*hds$POTASSIUM_min) <=10] <- "#63c683" #limegreen Quadrant 4
hds$hds_color[(2*hds$SODIUM_min) >= 200 & (2*hds$SODIUM_min) <= 270 & (2*hds$POTASSIUM_min) >=7.5 & (2*hds$POTASSIUM_min) <=10] <- "#7ec409" #brightergreen Quadrant 5
hds$hds_color[(2*hds$SODIUM_min) >= 270 & (2*hds$POTASSIUM_min) >=7.5 & (2*hds$POTASSIUM_min) <=10] <- "#f3a533" #orange Quadrant 6
hds$hds_color[(2*hds$SODIUM_min) <= 200 & (2*hds$POTASSIUM_min) >=10] <- "#e94152" #pink Quadrant 7
hds$hds_color[(2*hds$SODIUM_min) >= 200 & (2*hds$SODIUM_min) <= 270 & (2*hds$POTASSIUM_min) >=10] <- "#b33250" #marroon Quadrant 8
hds$hds_color[(2*hds$SODIUM_min) >= 270 & (2*hds$POTASSIUM_min) >=10] <- "#250511" #black #marroon Quadrant 9

g<-ggplot(hds, aes((2*POTASSIUM_min), (2*SODIUM_min))) +
  geom_point(color = hds$hds_color,
             alpha = 0.05,
             size = ((sqrt(((hds$GLUCOSE_min)/18)+((hds$BUN_min)/2.8))))) +
  ...
  geom_hline(aes(yintercept=200),
             ...
             linetype="dashed",
             size=0.5)+
  geom_vline(aes(xintercept=7.541543), 
             color="red", 
             alpha = 0.5, 
             size=0.75)+
  geom_hline(aes(yintercept=273.2512),
             color="red",
             alpha = 0.5,
             size=0.5) +
  geom_hline(aes(yintercept=275),
             ...
             alpha = 0.5,
             size=0.9) +
  geom_hline(aes(yintercept=295),
             color="#313695",
             alpha = 0.5,
             size=0.9) +
  theme_bw() +
  labs(x="Scaled Potassium (x2)", 
       y="Scaled Sodium (x2)",
       subtitlte="Osmolarity estimation (scale doubled)") +
  ggtitle("Assesment of osmolarity in patient population")
ggMarginal(g, type = "histogram", fill="transparent")
g

tiff("osmolarity.tiff", units="in", width=7.5, height=5.75, res=600)
g
dev.off()

jpeg("osmolarity.jpeg", units="in", width=7.5, height=5.75, res=600)
g
dev.off()
...
## PCA
### We perform PCA so that we can identify what features to focus on and use in following models. We validated the variables chosen and those removed based on clinical expertise. 
```{r}
...
Please note, balanced accuracy is the same as AUC for all of the models. 
...
We found it interesting that the Naive Bayes model seems to be robust to class imbalance, though this observation was unsurprising in conversations with peers. 
...
## for graphs
pred.rf<-predict.train(object=randomforest,dataTest,type="prob")%>% 
  bind_cols(dataTest)%>%
  select(X1, icustay_expire_flag)%>%
  mutate(Model="Random Forest")
```

The feature importance plot confirms the features otherwise identified in the logistic regression. There seemed to be varying results based on multiple iterations and potential complications with seeds, which is another reason we opted for the logistic regression as shown in our plots below.  
...
save(fit.svm_down, file="fit.svm.down.Rda")
...
It was interesting to see the similar performance of SVM with the logistic regression models, though as we did more research, recognize that we need to do more exploration to understand the linearity (or lack thereof) amongst the features in addition to other kernels. 
...
The logistic regression had a good balance of interpretability and accuracy, and from previous experience interacting with clinicians, believe that this will be among the easiest to practically implement given the general familiarity with regression compared to other machine learning techniques. This is an important consideration because this algorithm is intended to be a support tool, not a replacement, and therefore, interpretability and the option to further triage potential issues is critical in informing the decisions of the healthcare provider.
...
It was interesting to explore the basics of boosted trees with grid search and various other parameters, though we ultimately returned to the caret package for consistency of comparison between different models. 