...
plot1
# paths <- stringr::str_c("AgeDistribution",names(plot1), ".jpg")
...
hds<-hds %>% mutate(v=1,
            ...
            admission_location=paste("AdmissionLocation",admission_type,sep="_"))
...
hds<-left_join(hds3, hds)%>%
  select(-c(admission_type, admission_location))
...
```{r}
# features to delete
to_delete <- c("los_icu","los_hospital","intime","outtime","subject_id","hadm_id","icustay_id","admittime","religion","insurance","dischtime","ethnicity","admission_location","diagnosis","hospital_expire_flag","ICUSTAY_AGE_GROUP")
```

#### need to account for different admission locations - they should not be standardized 
